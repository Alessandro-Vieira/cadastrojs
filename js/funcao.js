...
function setList(cadastro){
	...
	for(var key in cadastro){
		table += '<tr><td></td><td>'+ formatNome(cadastro[key].nome) +'</td><td>'+ formatEmail(cadastro[key].email) +'</td><td>'+ formatStatus(cadastro[key].status) +'</td><td><img src="img/edit.png" width="30px" onclick="setUpdate('+key+');"><img src="img/trash.png" width="30px" onclick="deleteData('+key+');"></td></tr>';
	}
	table += '</tbody>';
